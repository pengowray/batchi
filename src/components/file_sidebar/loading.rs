...
use crate::audio::loader::{is_mp3, is_ogg, load_audio, parse_flac_header, parse_mp3_header, parse_ogg_header, parse_wav_header_with_file_size};
...
/// Returns Ok(()) if successful, Err if the file is not suitable for streaming.
async fn try_streaming_ogg(file: &File, name: &str, state: AppState) -> Result<(), String> {
    // Read first 64KB for header probing
    let header_size = 65536.0f64.min(file.size());
    let header_bytes = read_blob_range(file, 0.0, header_size).await?;
